{* $Id: fgal_listing_conf.tpl 49281 2013-12-31 10:04:22Z chibaguy $ *}
	...
	{foreach key=key item=item from=$fgal_options}
		{if $key eq 'default_view'}
            <div class="form-group">
                <label class="col-sm-4 text-right" for="fgal_{$key}">{$item.name}:</label>
                <div class="col-sm-8">
		...
		{else}
            <div class="form-group">
                <label class="col-sm-4 text-right" for="fgal_{$key}{if isset($fgal_ext)}{$fgal_ext}{/if}">{$item.name}:</label>
                <div class="col-sm-8">
                        {assign var='pref_name' value="fgal_$key"}
                         <input type="checkbox" id="fgal_{$key}" name="fgal_{$key}" {if $item.value eq 'y'}checked="checked"{/if}{if isset($edit_mode) and $edit_mode eq 'y' and $prefs.$pref_name neq 'y'} disabled="disabled"{/if}>
...
{if is_array($fgal_listing_conf) and count($fgal_listing_conf) gt 0}
	{foreach key=key item=item from=$fgal_listing_conf}
        <div class="form-group">
            <label class="col-sm-4 text-right"  for="fgal_list_{$key}">{$item.name}:</label>
            <div class="col-sm-8">
        		<select id="fgal_list_{$key}" name="fgal_list_{$key}">
		           {if $key eq 'name' or $key eq 'name_admin'}
			            <option value="a"{if isset($item.value) and $item.value eq 'a'} selected="selected"{/if}>{tr}Name-filename{/tr}</option>
			            <option value="n"{if isset($item.value) and $item.value eq 'n'} selected="selected"{/if}>{tr}Name{/tr}</option>
			            <option value="f"{if isset($item.value) and $item.value eq 'f'} selected="selected"{/if}>{tr}Filename only{/tr}</option>
	        	    {elseif $key neq 'deleteAfter'}
		            	<option value='n'{if isset($item.value) and $item.value eq 'n'} selected="selected"{/if}>{tr}Hide{/tr}</option>
		            	<option value='y'{if isset($item.value) and $item.value eq 'y'} selected="selected"{/if}>{tr}Show as a column{/tr}</option>
		        	    <option value='o'{if isset($item.value) and $item.value eq 'o'} selected="selected"{/if}>{tr}Show in popup box{/tr}</option>
		        	    <option value='a'{if isset($item.value) and $item.value eq 'a'} selected="selected"{/if}>{tr}Both{/tr}</option>
		            {/if}
		            {if $key eq 'lockedby' or $key eq 'lockedby_admin'}
			            <option value='i'{if isset($item.value) and $item.value eq 'i'} selected="selected"{/if}>{tr}Show an icon in a column{/tr}</option>
	        	    {/if}
