// $Id: tiki-jquery.js 52120 2014-07-26 11:53:46Z jonnybradley $
...
$.fn.tiki_popover = function () {
	...
	list = $.extend(list, $container.find("a[data-toggle=popover]"));
	...
	$container.find('.ajaxtips').popover({
		trigger: 'hover',
		html: true,
		content: function () {
...
(function($) {
	...
	$.fn.object_selector = function (prepareOptionFn) {
		...
		this.each(function () {
			var input = this, filter = $(input).data('filters'), threshold = $(input).data('threshold');
			$(input).hide();

			var $spinner = $(this).parent().tikiModal(" "),
				$select = $(input).closest('.object-selector').find('select').first(),
				$filter = $(input).closest('.object-selector').find(':text.filter').first(),
				$search = $(input).closest('.object-selector').find('button.search').first(),
				$panel = $(input).closest('.object-selector').find('.panel').first(),
				$selection = $(input).closest('.object-selector').find('.panel-footer .selection').first();
			...
			$.getJSON('tiki-searchindex.php', {
				...
				$selection.text($(input).data('title'));
				...
				if ($select.children().size() <= threshold) {
					...
					$search.click(function (e) {
						...
						this.ajax = $.getJSON('tiki-searchindex.php', {
							filter: $.extend(filter, {title: $filter.val()})
						}, function (data) {
							var $results = $filter.closest('.object-selector').find('.results');
							var $noresults = $filter.closest('.object-selector').find('.no-results');
							$results.empty();
							$noresults.removeClass('hidden');

							$.each(data, function (key, value) {
								var item = $('<div class="radio"><label><input type="radio"> </label></div>');
								$('label', item).append(value.title);
								$(':radio', item)
									.attr('name', input.id + "_sel")
									.val(value.object_type + ':' + value.object_id)
									.click(function () {
										if ($(this).is(':checked')) {
											$(input)
												.data('label', value.title)
												.val($(this).val()).change();
											$selection.text(value.title);
										}
									});
								$results.append(item);
		...
		this
			.each(function () {
				...
				$(this).prepend($('<span />').attr({
					'class' : isFavorite() ? 'glyphicon glyphicon-star' : 'glyphicon glyphicon-star-empty',
...
$(function () {
	if (document.location.hash) {
		$('a[data-toggle=tab][href="' + document.location.hash + '"]').tab('show');
	}
});
...
$.closeModal = function (options) {
	$('#bootstrap-modal')
		.one('hidden.bs.modal', options.done || function () {})
		.modal('hide');
}
